...
if __name__ == "__main__":
    main()        if normalized[column].dtype == object:
            normalized[column] = normalized[column].map(
                lambda value: value.strip() if isinstance(value, str) else value
            )
...
def render_sidebar() -> tuple[str, str, bool]:
    ...
    has_header = st.sidebar.checkbox("La premiere ligne contient les en-tetes", value=True)
    return source_mode, ("\t" if delimiter == "\\t" else delimiter), has_header
...
def render_intro() -> None:
    ...
    with st.expander("Ce que fait cette version"):
        st.markdown(
            ...
            Cette premiere version repose sur des heuristiques locales. L'etape IA pourra etre ajoutee ensuite.
            """
        )
...
def main() -> None:
    ...
    source_mode, delimiter, has_header = render_sidebar()
